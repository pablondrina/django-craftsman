...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options["clear"]:
            ...
            RecipeInput.objects.all().delete()
    ...
    def _print_summary(self):
        ...
        # Step field stats
        with_mixing = WorkOrder.objects.filter(mixing__isnull=False).count()
        with_produced = WorkOrder.objects.filter(
            produced_quantity__isnull=False
        ).count()
        self.stdout.write(f"\n📈 Campos preenchidos:")
        self.stdout.write(f"   • mixing: {with_mixing}")
        self.stdout.write(f"   • produced_quantity: {with_produced}")
    ...
    def _create_recipes(self, products, work_center):
        ...
        for product in products:
            ...
            if existing:
                if existing.production_stages != ["Mixing", "Shaping", "Baking"]:
                    existing.production_stages = ["Mixing", "Shaping", "Baking"]
                    existing.save()
